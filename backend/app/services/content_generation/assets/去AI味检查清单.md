...
### 6.5 不必要的标点符号 🚫
识别信号：大量使用""《》——「」等标点符号。这些符号在公众号排版中显得臃肿，影响观感和阅读流畅度。
改写（硬性禁止）：
- 「」：直接写名称即可，如「AI 团队」→ AI 团队
- "" "" '' ''（双引号/单引号）：删掉，直接写内容。只有在必须区分引用和叙述时才保留
- 《》（书名号）：改成直接写名称，如《美图设计室》→ 美图设计室
- ——（破折号）：删掉或改成逗号/句号分段
- ……（省略号）：尽量少用，最多 1 处，能用句号/逗号替代就替代
- 我们不是新闻报纸，不需要这些正式排版符号，公众号文章更重要的是排版美观、阅读流畅
...
11. 段落开头 emoji（📌 🎯 💡 等）
12. 每段都有加粗（导致加粗失效）
13. 大量使用""《》——等不必要的标点符号（公众号文章不需要新闻报纸式排版）